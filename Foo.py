...
class Foo():
    ...
    def think(self, question):
        ...
        try:
            return "Вопрос уже был, ответ был %s" % self.memory[words]
        except KeyError:
            try:
                answer = self.__fuzzy_key_search(words)
                return "Вопрос уже был, ответ был %s." % answer
            except (AssertionError, ZeroDivisionError):
                answer=self.answers[randint(0,len(answers)-1)]
                self.memory.update({words:answer})
                return "Ответ - %s" % answer
            
if __name__ == '__main__':
    f = Foo()
    print(f.think('Какой-то вопрос?'))
    print(f.think('Какой-то вопрос?'))
    print(f.think('Какой-то вопросик?'))
    print(f.think('Какой-то вопросчек?'))
